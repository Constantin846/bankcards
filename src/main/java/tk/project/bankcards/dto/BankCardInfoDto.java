...
@Schema(description = "DTO для информации о банковской карте")
public record BankCardInfoDto(
    ...
    @Schema(description = "Номер карты", example = "**** **** **** 1234", requiredMode = REQUIRED)
        String number,
    @Schema(description = "Владелец карты", requiredMode = REQUIRED) UserInfoDto owner,
    ...
    @Schema(description = "Баланс карты", requiredMode = REQUIRED) BigDecimal balance) {}
